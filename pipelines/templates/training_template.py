...
class Training(FlowSpec, DatasetMixin):
    ...
    def start(self):
        ...
        try:
            ...
            # recognize how they relate to each other. And the current variable gives us
            # access to the current run, so we can use it to get the run ID.
            run = mlflow.start_run(run_name=current.run_id)
            # Create an artifact that stores the metaflow run id
    ...
    def evaluate_fold(self):
        ...
        # Retain variables for calculating precision and recall
        self.y_test_fold = y_test
        self.predictions_fold = predictions
        ...
        self.precision = precision_score(y_test, predictions, average='macro')
        self.recall = recall_score(y_test, predictions, average='macro')
    ...
    def average_scores(self, inputs):
        ...
        precision_metrics = [i.precision for i in inputs]
        recall_metrics = [i.recall for i in inputs]
        self.precision = np.mean(precision_metrics)
        self.recall = np.mean(recall_metrics)
        # self.recall = np.mean([recall_score(y_test, predictions, average='macro') for y_test, predictions in zip(y_test_metrics, predictions_metrics)])
        ...
        logging.info("Precision: %s", self.precision)
        logging.info("Recall: %s", self.recall)
